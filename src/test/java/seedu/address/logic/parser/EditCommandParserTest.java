...
public class EditCommandParserTest {
    ...
    public void parse_invalidValue_failure() {
        ...
        assertParseFailure(parser, "1" + INVALID_APPLICATION_DATE_DESC, ApplicationDate
                .MESSAGE_CONSTRAINTS); // invalid application date
        assertParseFailure(parser, "1" + INVALID_ADDRESS_DESC, Address.MESSAGE_CONSTRAINTS); // invalid address
        assertParseFailure(parser, "1" + INVALID_TAG_DESC, Tag.MESSAGE_CONSTRAINTS); // invalid tag

        // invalid phone followed by valid application date
        assertParseFailure(parser, "1" + INVALID_PHONE_DESC + APPLICATION_DATE_DESC_AMY, Phone.MESSAGE_CONSTRAINTS);
        ...
        assertParseFailure(parser, "1" + INVALID_COMPANY_DESC + INVALID_APPLICATION_DATE_DESC + VALID_ADDRESS_AMY
                        + VALID_PHONE_AMY,
    ...
    public void parse_allFieldsSpecified_success() {
        ...
        String userInput = targetIndex.getOneBased() + PHONE_DESC_BOB + TAG_DESC_HUSBAND
                + APPLICATION_DATE_DESC_AMY + ADDRESS_DESC_AMY + COMPANY_DESC_AMY + TAG_DESC_FRIEND;

        EditApplicationDescriptor descriptor = new EditApplicationDescriptorBuilder().withCompany(VALID_COMPANY_AMY)
                .withPhone(VALID_PHONE_BOB).withApplicationDate(VALID_APPLICATION_DATE_AMY)
    ...
    public void parse_someFieldsSpecified_success() {
        ...
        String userInput = targetIndex.getOneBased() + PHONE_DESC_BOB + APPLICATION_DATE_DESC_AMY;
    ...
    public void parse_multipleRepeatedFields_failure() {
        ...
        userInput = targetIndex.getOneBased() + PHONE_DESC_AMY + ADDRESS_DESC_AMY + APPLICATION_DATE_DESC_AMY
                + TAG_DESC_FRIEND + PHONE_DESC_AMY + ADDRESS_DESC_AMY + APPLICATION_DATE_DESC_AMY + TAG_DESC_FRIEND
                + PHONE_DESC_BOB + ADDRESS_DESC_BOB + APPLICATION_DATE_DESC_BOB + TAG_DESC_HUSBAND;

        assertParseFailure(parser, userInput,
                Messages.getErrorMessageForDuplicatePrefixes(PREFIX_PHONE, PREFIX_APPLICATION_DATE, PREFIX_ADDRESS));

        // multiple invalid values
        userInput = targetIndex.getOneBased() + INVALID_PHONE_DESC + INVALID_ADDRESS_DESC
                + INVALID_APPLICATION_DATE_DESC
                + INVALID_PHONE_DESC + INVALID_ADDRESS_DESC + INVALID_APPLICATION_DATE_DESC;
